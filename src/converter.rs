...
use crate::n2p::{ConversionConfig, NotionBlockVisitor, NotionToPandocVisitor};
...
use crate::notion_block_putter::{create_debug_block_putter, BlockPutterError};
...
use pandoc_types::definition::{Pandoc, Block as PandocBlock};
use std::path::Path;
use std::fs;
...
impl NotionConverter {
    ...
    pub fn configure_notion_client(&mut self, token: String) -> Result<(), ConversionError> {
        let client = notion_client::endpoints::Client::new(token, None)
            .map_err(|e| e)?;
        ...
        for (i, block) in blocks.iter().enumerate() {
            match block {
                PandocBlock::Plain(inlines) => {
                    println!("{}Block #{}: Plain with {} inlines", indent_str, i, inlines.len());
                },
                PandocBlock::Para(inlines) => {
                    println!("{}Block #{}: Para with {} inlines", indent_str, i, inlines.len());
                },
                PandocBlock::LineBlock(lines) => {
                    println!("{}Block #{}: LineBlock with {} lines", indent_str, i, lines.len());
                },
                PandocBlock::CodeBlock(attr, text) => {
                    println!("{}Block #{}: CodeBlock '{}' ({} chars)", indent_str, i, attr.classes.join(", "), text.len());
                },
                PandocBlock::RawBlock(format, text) => {
                    println!("{}Block #{}: RawBlock format '{:?}' ({} chars)", indent_str, i, format, text.len());
                },
                PandocBlock::BlockQuote(blocks) => {
                    println!("{}Block #{}: BlockQuote with {} blocks", indent_str, i, blocks.len());
                    self.debug_print_pandoc_blocks(blocks, indent + 4);
                },
                PandocBlock::OrderedList(_attrs, items) => {
                    println!("{}Block #{}: OrderedList with {} items", indent_str, i, items.len());
                ...
                PandocBlock::BulletList(items) => {
                    println!("{}Block #{}: BulletList with {} items", indent_str, i, items.len());
                    for (j, item) in items.iter().enumerate() {
                        println!("{}  Item #{} with {} blocks", indent_str, j, item.len());
                        self.debug_print_pandoc_blocks(item, indent + 8);
                    }
                ...
                PandocBlock::DefinitionList(items) => {
                    println!("{}Block #{}: DefinitionList with {} items", indent_str, i, items.len());
                },
                PandocBlock::Header(level, _attr, inlines) => {
                    println!("{}Block #{}: Header level {} with {} inlines", indent_str, i, level, inlines.len());
                },
                ...
                PandocBlock::Div(attr, blocks) => {
                    println!("{}Block #{}: Div '{}' with {} blocks", indent_str, i, attr.classes.join(", "), blocks.len());
                    self.debug_print_pandoc_blocks(blocks, indent + 4);
                },
                PandocBlock::Null => {
                ...
                PandocBlock::Figure(_attr, _caption, blocks) => {
                    println!("{}Block #{}: Figure with {} blocks", indent_str, i, blocks.len());
                    self.debug_print_pandoc_blocks(blocks, indent + 4);
                },
            }
        }
    ...
    fn debug_save_to_file<T: serde::Serialize>(&self, data: &T, filename: &str) -> Result<(), String> {
    ...
    pub async fn notion_blocks_to_pandoc(
        &self, 
        block_id: &str
    ) -> Result<Pandoc, ConversionError> {
        ...
        let blocks = block_fetcher.fetch_block_with_children(block_id).await
        ...
        // Process each block with the visitor
        let mut pandoc_blocks = Vec::new();
        for block in &blocks {
            // NotionToPandocVisitor.visit_block returns Vec<PandocBlock> directly
            let block_results = visitor.visit_block(block);
            pandoc_blocks.extend(block_results);
        }
    ...
    pub async fn notion_page_to_file<P: AsRef<Path>>(
        ...
        self.processor.ast_to_file_with_format(&pandoc, output_path, format)?;
    ...
    pub async fn upload_blocks_to_notion(
        ...
        self.upload_blocks_level_order(parent_id, blocks, client.clone()).await
    }
    ...
    async fn upload_blocks_level_order(
        ...
        block_putter.upload_blocks(parent_id, blocks)
            .await
    ...
    pub async fn notion_to_file<P: AsRef<Path>>(
        ...
        self.notion_page_to_file(block_id, output_path, format).await?;
